...
public class TextReportModel6 extends FTPRealseModel implements Text_Report{
	...
	public void Write_text(String CustomerCode, String Device, String Lot, String CP, File DataSorce,String FileName)
		...
		for (int k = 0; k < Filelist.length; k++) {				
			...
			File directory=new File("/TestReport/CustReport/"+CustomerCode+"/"+Device+"/"+Lot+"/"+CP+"/"+Lot+"-"+CP);
			if (!directory.exists()) {
				directory.mkdirs();
			}
			File Result_Text=new File("/TestReport/CustReport/"+CustomerCode+"/"+Device+"/"+Lot+"/"+CP+"/"+Lot+"-"+CP+"/"+FinalName);
			...
			out.print("\r\n");
			out.print("[ WAFER MAP]"+"\r\n");
			for (int i = TestDie_MinY_R-3; i < TestDie_MaxY_R+3; i++) {
