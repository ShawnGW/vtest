...
public class CustomerReport {
	...
	private final File DataSource=new File("/TestReport/RawData");
										...
										for (int l = 0; l < CPProcess.length; l++) {
											try {
												if (CPProcess[l].isDirectory()&&CPProcess[l].listFiles().length>0) {
													...
													File Backup=new File("/TestReport/RawData_Backup/"+CustomerCode+"/"+Device+"/"+Lot+"/"+CP);
													if (!Backup.exists()) {
														Backup.mkdirs();
													}
													File[]  RawDatas=CPProcess[l].listFiles();
													for (File Rawdata : RawDatas) {
														File Backfile=new File("/TestReport/RawData_Backup/"+CustomerCode+"/"+Device+"/"+Lot+"/"+CP+"/"+Rawdata.getName());
														if (Backfile.exists()) {
															Backfile.delete();
														}
													...
													File Report_Directory=new File("/TestReport/CustReport/"+CustomerCode+"/"+Device+"/"+Lot+"/"+CP);
													...
													try {
														...
														for (int m = 0; m < Excel_Models.length; m++) {
															...
															if (ModelMap.containsKey(Excel_Models[m])) 
																...
																ExcelName="LOT.xlsx";
															File Result_Excel=new File("/TestReport/CustReport/"+CustomerCode+"/"+Device+"/"+Lot+"/"+CP+"/"+ExcelName);
