...
public class TelP8ProberMappingGenerateRawdata {
	private final File dataSourceP=new File("/prod/prober/TEL/P8/mapup");
	private final File dataSourcePMapdown=new File("/prod/prober/TEL/P8/mapdown");
	boolean smallDie=false;
	public static void main(String[] args) throws IOException {
		TelP8ProberMappingGenerateRawdata tel=new TelP8ProberMappingGenerateRawdata();
		tel.generate();
	}
					...
					if (smallDie) {
						for (File lotDa : lotDaArray) {
							...
							for (File waferIdDa : waferIdDaArray) {
								...
								File proberMappingBackUpDirectory=new File("/prod/mapbackup/"+customerCode+"/"+device+"/"+customerLot+"/"+cp);
								if (!proberMappingBackUpDirectory.exists()) {
									proberMappingBackUpDirectory.mkdir();
								}
								waferIdDaName=waferIdDaName+"_"+(new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()));
								File waferIdDaBackupFile=new File("/prod/mapbackup/"+customerCode+"/"+device+"/"+customerLot+"/"+cp+"/"+waferIdDaName);
								try {
									FileUtils.copyFile(waferIdDa, waferIdDaBackupFile);
								} catch (Exception e) {
									// TODO: handle exception
								}
							}
							...
							lotDaName=lotDaName+"_"+(new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()));
							File lotDaBackFile=new File("/prod/mapbackup/"+customerCode+"/"+device+"/"+customerLot+"/"+cp+"/LOT/"+lotDaName);
							FileUtils.copyFile(lotDa, lotDaBackFile);
							if (WAFCONT!=null) {
								...
								File WAFCONTBackFile=new File("/prod/mapbackup/"+customerCode+"/"+device+"/"+customerLot+"/"+cp+"/WAFCONT/"+WAFCONTName);
								FileUtils.copyFile(WAFCONT, WAFCONTBackFile);	
							}
							if (FORM!=null) {
								File FORMBackUpDirectory=new File("/prod/mapbackup/"+customerCode+"/"+device+"/"+customerLot+"/"+cp+"/FORM/");
								if (!FORMBackUpDirectory.exists()) {
									FORMBackUpDirectory.mkdir();
								}
								String FORMName=FORM.getName();
								FORMName=FORMName+"_"+(new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()));
								File FORMBackFile=new File("/prod/mapbackup/"+customerCode+"/"+device+"/"+customerLot+"/"+cp+"/FORM/"+FORMName);
								FileUtils.copyFile(FORM, FORMBackFile);	
							}
						}
					}else
					{
						for (File lotDat : lotDatArray) {
							...
							lotDatName=lotDatName+"_"+(new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()));
							File proberMappingBackFile=new File("/prod/mapbackup/"+customerCode+"/"+device+"/"+customerLot+"/"+cp+"/LOT/"+lotDatName);
