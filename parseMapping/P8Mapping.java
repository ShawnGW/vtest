...
public class P8Mapping {
		public static void main(String[] args) throws IOException {
			...
			P8Mapping p8Mapping=new P8Mapping();
			HashMap<String, String> resultMap=p8Mapping.Get(new File("D:/BFSCSV/001.HV7GVPG"),0,Bin_summary_Map,DieMap,skipAndMarkDieMap);
...
//			String Lot=null;
//			byte[] Lot_BT=new byte[18];
			String Test_Start_Time=null;
			byte[] Test_Start_Time_BT=new byte[12];
			String Test_End_Time=null;
			byte[] Test_End_Time_BT=new byte[12];
//			String Wafer_Load_Time=null;
//			byte[] Wafer_Load_Time_BT=new byte[12];	
//			String Wafer_Unload_Time=null;
//			byte[] Wafer_Unload_Time_BT=new byte[12];	
...
//				if (i>171&&i<184) {
//					Wafer_Load_Time_BT[i-172]=bs[i];
//				}
//				if (i>183&&i<196) {
//					Wafer_Unload_Time_BT[i-184]=bs[i];
//				}
				if (i>213&&i<216) {
...
//			Lot=new String(Lot_BT);
			
			Test_Start_Time=new String(Test_Start_Time_BT);
			Test_End_Time=new String(Test_End_Time_BT);
//			Wafer_Load_Time=new String(Wafer_Load_Time_BT);
//			Wafer_Unload_Time=new String(Wafer_Unload_Time_BT);
...
//					int dieMarking=(Integer.parseInt(Integer.toHexString(bs[i+1]&0xFF),16)/32)&1;
					...
					/**  get m x,y limit **/
					if (dieExcept!=1){
						if (!DieMap.containsKey(CellX+":"+CellY)&&!skipAndMarkDieMap.containsKey(CellX+":"+CellY)) {
							if (Die_property==0)
							{
								skipAndMarkDieMap.put(CellX+":"+CellY, "S");
							}
							else if (Die_property==2)
							{
								skipAndMarkDieMap.put(CellX+":"+CellY, "M");
							}
							else if (Die_property==1)
							{
								...
								DieMap.put(CellX+":"+CellY, ""+measureFlag);
								if (dieTestResult==1) {
									PassDie++;
								}
							}
